...
const EditEventPublishForm = () => {
  ...
  useEffect(() => {
    setEvent(data?.singleEvent?.event);
  }, [data, loading]);
        ...
        <form>
          <div className="create-event-publish">
            ...
            <div className="event-publish-date">
              ...
              <p>
                {event?.start_date?.length > 0 && formatDate(event?.start_date)}{" "}
                at{" "}
                {event?.start_time?.length > 0 && formatTime(event?.start_time)}
              </p>
...
export default EditEventPublishForm;
